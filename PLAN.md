...
- [x] `pytest` (no args) must exit green with coverage ≥ 70%
- [x] `pytest -m security -v` must exit green independently

**Checkpoint:** `pytest` exits green; coverage ≥ 70%; `pytest -m security` all pass.
...
- [x] `.github/workflows/ci.yml` — triggers on every `push` and `pull_request`
  ...
  - Runs `pytest` — fails if coverage < 70% or any test fails
...
- [ ] Configure `lib/api.ts` (fetch wrapper with base URL + Authorization header)
- [ ] Configure `middleware.ts` (protect `/orders` and `/admin` routes)
...
**Checkpoint (full):** visitor browses `/` → registers → creates order → sees status badge; admin logs in → sees all orders → advances status → Vercel deploy live.
...
| D-4 | Frontend stack? | Next.js 14 + TypeScript + Tailwind + shadcn/ui |
| D-5 | Frontend deploy target? | Vercel |
